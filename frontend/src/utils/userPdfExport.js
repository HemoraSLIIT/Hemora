...
function buildContentStream(title, rows, pageNumber, totalPages, hasLogo) {
  ...
  const columns = [
    ...
    { key: "name", label: "Name", x: 92 },
    { key: "email", label: "Email", x: 240 },
    { key: "role", label: "Role", x: 420 },
  ...
  rows.forEach((row) => {
    const values = {
      ...
      name: truncate(row.name || "Unknown", 25),
      email: truncate(row.email || "-", 32),
      role: truncate(row.role || "-", 14),
  ...
  lines.push("BT");
  lines.push("/F1 10 Tf");
  lines.push(`50 30 Td (${escapePdfText(`Total users: ${rows.length}`)}) Tj`);
  lines.push("ET");

  lines.push("BT");
  lines.push("/F1 10 Tf");
  lines.push(`512 30 Td (${escapePdfText(`Page ${pageNumber} of ${totalPages}`)}) Tj`);
  lines.push("ET");

  lines.push("BT");
  lines.push("/F1 10 Tf");
  lines.push(`${poweredByX} 30 Td (${escapePdfText(poweredByLabel)}) Tj`);
  lines.push("ET");
...
export async function downloadUsersPdf(filename, title, rows) {
  let brandLogo = null;
